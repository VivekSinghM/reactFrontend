...
const Table=props=>{
    const badgeBg=props.isOccupied?"badge-success":"badge-secondary"
    let itemsJSX= <h2 className='text-secondary'>No Order</h2>
    if (!!props.order) {itemsJSX= Object.entries(props.order).map(([name,qty])=>{
        ...
        <div className='p-2' style={{width:'270px'}}>
            <div className="card">
                <div className="card-header p-2">
                    ...
                    <h3 className="m-0">T-{Number(props.tId).toLocaleString('en-US', {minimumIntegerDigits: 2,useGrouping: false})}</h3> <p className={"badge " + badgeBg} style={{margin:'auto',marginRight:0}}>{props.isOccupied?'Occupied':'Free'}</p>
                    </div>
                </div>
                <div className="card-body p-2">
                    ...
                    <div className='row m-auto'>
                        <div className='col p-0 d-flex'><button className="btn btn-dark border-0 p-1">{props.isOccupied?"Generate bill":"Place order"}</button></div>
                        <div className="d-flex align-items-center "><span><Link to={''}>open..</Link></span></div> 
                    </div>
